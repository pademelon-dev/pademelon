...
"""
    with git_repo({fname: bad},
                  {'.isort.cfg': u'', ofname: bad}) as fake_git:
        runner = CliRunner()
        # Exercise
        fullargs = [
            '--no-show', '--upstream-branch', fake_git.upstream_branch,
        ]
        result = runner.invoke(main, fullargs)
    # Verify
    expected = 'fake.py:2:10: E261 at least two spaces before inline comment'
    assert expected in result.output  # nosec
    assert len(result.output.splitlines()) == 1  # nosec
